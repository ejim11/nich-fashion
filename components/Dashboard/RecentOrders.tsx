...
const RecentOrders = ({ isLoading }: { isLoading: boolean }) => {
  ...
  return (
    <section className="bg-white rounded-[2rem] border border-[rgba(248,249,250,1)] flex flex-col px-[3rem] py-[2.3rem] shadow-custom-4 my-[2.4rem]">
      <h3 className="capitalize text-[2rem] mb-[3rem] font-bold font-satoshi">
        recent orders
      </h3>
      <div className="w-full">
        <TableComp tableHeaders={tableHeaders}>
          {recentOrders
            .map(
              (order: {
                id: string;
            ...
            .map((or, i) => (
              <tr key={i} className="border-b border-b-[rgba(239,241,243,1)] ">
                <td className="px-[1rem] py-[1rem] ">#{or.id.split("-")[0]}</td>
                <td>{or.customer}</td>
